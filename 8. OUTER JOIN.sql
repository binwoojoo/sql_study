...
-- 회원정보(user)와 회원의 주문 내역(order)을 조회
...
SELECT
    *
FROM tb_emp E
LEFT OUTER JOIN tb_dept D  -- 매칭이 안되면 전부다 null로 처리함 OUTER 생략 가능
ON e.dept_cd = d.dept_cd
;

-- 오라클 조인
-- LEFT: 조인 조건 오른쪽에 (+)를 붙임
-- RIGHT: 조인 조건 왼쪽에 (+)를 붙임
-- FULL: 지원하지 않음
...
SELECT
    *
FROM tb_emp E
RIGHT OUTER JOIN tb_dept D -- OUTER 생략 가능
ON e.dept_cd = d.dept_cd
;
