...
SELECT -- pk로 필터링하면 무조건 1건 이하가 조회됨
    emp_no,
    emp_nm,
    addr,
    sex_cd
...
SELECT
    ...
    sex_cd
FROM tb_emp
WHERE sex_cd <> 2 -- <> 또는 !=: 값이 다름을 나타냅니다.
;
...
SELECT
    ...
    birth_de
FROM tb_emp
WHERE birth_de >= '19800101' -- 1980년대생들만 조회
...
SELECT
    ...
    addr
FROM tb_emp
WHERE emp_nm LIKE '이__' -- '이'로 시작하는 세글자
;
...
SELECT
    ...
    addr
FROM tb_emp
WHERE emp_nm LIKE '%심' -- '심'으로 끝나는
;

SELECT
    email
FROM user
WHERE email LIKE '_A%@%' -- 두번 째 글자가 A여야함 banana@gmail.com
;
